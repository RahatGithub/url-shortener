...
const Dashboard = () => {
    ...
    const truncateUrl = (url, maxLength = 50) => url.length <= maxLength ? url : url.substring(0, maxLength) + '...';
    ...
    return (
        <div className="min-h-screen bg-gray-100 py-8">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-xl shadow-md p-6 mb-8">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-gray-600">{urlStats.totalUrls} / {urlStats.maxUrls} URLs used</p>
                        </div>
                        <div className="w-64">
                            <div className="bg-gray-200 rounded-full h-4">
                                <div 
                                    className={`h-4 rounded-full ${urlStats.remainingUrls <= 10 ? 'bg-red-500' : 'bg-blue-600'}`}
                    ...
                    {urlStats.remainingUrls <= 10 && (
                        <div className="mt-4 flex items-center bg-yellow-50 text-yellow-800 p-4 rounded-lg">
                            ...
                            <span>
                                ...
                                <button className="font-semibold underline ml-1">Upgrade to Premium</button> for unlimited URLs!
                            </span>
                        </div>
                    )}
                </div>
                ...
                {/* Create New URL form */}
                <div className="bg-white rounded-xl shadow-md p-6 mb-8">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">Create New Short URL</h3>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {urls.map((url) => (
                                        <tr key={url.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4">
                                                ...
                                                >
                                                    <span className="text-gray-900">
                                                        {expandedUrl === url.id ? url.originalUrl : truncateUrl(url.originalUrl)}
                                                    </span>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-mono">{url.shortCode}</span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <a href={url.shortUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
                                                    {url.shortUrl}
                                            ...
                                            <td className="px-6 py-4 text-gray-500">{formatDate(url.createdAt)}</td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <button onClick={() => copyToClipboard(url.shortUrl)} className="p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Copy to clipboard"><FiCopy /></button>
                                                    <button onClick={() => handleDelete(url.id)} className="p-2 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition" title="Delete"><FiTrash2 /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
